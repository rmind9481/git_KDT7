...
# 1.2 continent의 값이 Europe인 country, capital, top_level_domain 출력 
select country, capital, top_level_domain from world 
where continent='Europe';

# 1.3 Asia 국가의 국가명, 인구, 수도명을 인구가 많은 순서로 출력 

select country, population from world 
where continent='Asia' order by population desc;

# 1.4 전체 국가의 면적(area)을 내림차순으로 정렬하여 출력 
SELECT country, continent, area from world 
order by area desc;

# 1.5 전체 국가의 국가명, top_level_domain 을 오름차순으로 정렬하여 출력 
