...
def display_company_name(top_company_list):
    '''
    메뉴 출력: 리스트에 저장된 코스피 상위 10대 기업의 이름을 출력
    '''
    print('-------------------------------------')
...
def main():
    ...
    company_url_list = [] # top10 회사의 Naver url 정보
    company_name_list = [] # top10 회사의 이름

    get_top10_kospi_index(naver_stock_url, company_url_list, company_name_list)

    while True:
        display_company_name(company_name_list)
        choice = int(input('주가를 검색할 기업의 번호를 입력하세요(-1: 종료): '))
        if choice == -1:
            break
        elif choice >=1 and choice <=10:
            # 선택한 기업의 url로 이동
            get_company_info(company_url_list[choice-1])
        else:
